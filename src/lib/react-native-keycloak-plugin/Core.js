...
import { ErrorHttpTypesEnum } from '../../services/errors';
...
const retrieveTokens = async (conf, code, resolve, reject, deepLinkUrl) => {
  const { resource, credentials, realm, redirectUri, 'auth-server-url': authServerUrl } = conf;
...
const performLogin = async (conf, username, password, scope = 'info') => {
  ...
  //   type: 'info',
  //   icon: 'auto',
  //   message: JSON.stringify({ url, options }, undefined, 4),
  //   autoHide: false,
  // });
  logToConsole({ url, options });
  const fullResponse = await fetch(url, options);
  const jsonResponse = await fullResponse.json();
  logToConsole(fullResponse);
  if (fullResponse.status === 200) {
    console.log('????');
    await TokenStorage.saveConfiguration(conf);
    await TokenStorage.saveTokens(jsonResponse);
  ...
  if (fullResponse.status === 401) {
    throw {
      type: ErrorHttpTypesEnum.NetworkError,
...
export const keycloakUILogin = (conf, callback, scope = 'info') =>
