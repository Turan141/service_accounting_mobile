...
import type { RNFetchBlobNative, RNFetchBlobConfig, RNFetchBlobStream, RNFetchBlobResponseInfo } from './types';
  ...
  constructor(taskId: string, info: RNFetchBlobResponseInfo, data: any) {
    ...
    this.json = (): any => {
      switch (this.type) {
        ...
        case 'path':
          return fs.readFile(this.data, 'utf8').then(text => Promise.resolve(JSON.parse(text)));
        default:
