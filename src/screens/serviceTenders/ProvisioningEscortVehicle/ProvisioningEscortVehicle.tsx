...
import ProvisioningEscortVehicleReport from './ProvisioningEscortVehicleReport';
...
const ProvisioningEscortVehicleScreen: FC<TenderScreenProps> = ({ navigation }) => {
  ...
  const stepperSteps: TaskStepSchema[] = useMemo(
    () => [
      ...
      {
        order: 2,
        label: 'Предоставление автомашины для сопровождения при буксировке ВС через ВПП',
  ...
  const formControls = useForm<AircraftCoolingFormValues>({
    mode: 'onSubmit',
  ...
  return (
    <>
      ...
      >
          ...
          <View style={{ padding: 20 }}>
            ...
            {errors.forcedDownTime ? (
              <InlineAlert type="danger">
                Чтобы начать Охлаждение ВС, сначала отожмите кнопку «Вынужденный простой»
              </InlineAlert>
            ) : null}
        ...
        {currentStep === TenderStepperKeys.Result && <ProvisioningEscortVehicleReport />}
      </ScrollViewContainer>
    </>
  );
};

export default observer(ProvisioningEscortVehicleScreen);
