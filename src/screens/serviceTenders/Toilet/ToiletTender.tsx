...
import ToiletTenderReport from './ToiletTenderReport';
...
const ToiletTenderScreen: FC<TenderScreenProps> = ({ navigation, route }) => {
  const { currentTender, addPositionsToTender, editItemOfDocument, cancelService, changeItemStatus, loading } =
    useTendersStore();
  const { getDictionaryItemsByType, toiletOptions } = useDictionariesStore();
  ...
  const stepperSteps: TaskStepSchema[] = useMemo(
    () => [
      ...
      {
        order: 2,
        label: 'Обработка системы санузла',
  ...
  const tenderItemName = DocumentItemNamesEnum.ToiletType;

  const formControls = useForm<ToiletTenderFormValues>({
    mode: 'onSubmit',
    ...
    defaultValues: {
      ...
      canCancelService: !currentTender?.items?.find(item => item.type === DocumentItemNamesEnum.ToiletType).properties
        ?.started,
      [tenderItemName]: currentTender?.items?.find(item => item.type === DocumentItemNamesEnum.ToiletType),
      itemAdditionalInfo: currentTender?.items?.find(item => item.type === DocumentItemNamesEnum.ToiletType)
        .additionalInfo,
  ...
  const handleChangeToiletType = useCallback(
    async (currentValue: DocumentItemView, option: DocumentItemView, onChange: (value: DocumentItemView) => void) => {
      editItemOfDocument({
        referenceMasterCode: option.masterCode,
        id: currentValue.id,
      })
        .then(items => onChange(items.find(item => item.type === tenderItemName)))
        .then(() => console.log('Item after edit', watch('toiletItem')));
    },
    []
  );

  const handleToiletTimer = useCallback(async () => {
  ...
  return (
    <>
      ...
      >
          ...
          <View style={{ padding: 20 }}>
            ...
            {errors.forcedDownTime ? (
              <InlineAlert type="danger">
                Чтобы начать Обработка системы санузла, сначала отожмите кнопку «Вынужденный простой»
              </InlineAlert>
            ) : null}
              ...
              <Controller
                ...
                render={({ field: { onChange, value } }) => (
                  <Select
                    ...
                    label={'Тип'}
                    items={toiletOptions}
                    onSelect={(option: DocumentItemView) => handleChangeToiletType(value, option, onChange)}
                  />
                )}
              />
            </FormGroup>

            {watch(tenderItemName).masterCode === 'other' && (
              <FormGroup>
                <Controller
                  control={control}
                  rules={{
                  ...
                  render={({ field: { onChange, value } }) => (
                    <TextInput
                      ...
                      value={value}
                      onChangeText={onChange}
                      onSubmitEditing={() => additionalInfoSubmitHandler(value, tenderItemName)}
                    />
                  )}
                />
              </FormGroup>
        ...
        {currentStep === TenderStepperKeys.Result && <ToiletTenderReport />}
      </ScrollViewContainer>
    </>
  );
};

export default observer(ToiletTenderScreen);
