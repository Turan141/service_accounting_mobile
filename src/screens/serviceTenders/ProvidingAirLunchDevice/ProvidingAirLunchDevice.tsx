...
import ProvidingAirLunchDeviceReport from './ProvidingAirLunchDeviceReport';
...
const ProvidingAirLunchDeviceScreen: FC<TenderScreenProps> = ({ navigation }) => {
  ...
  const tenderItemName = DocumentItemNamesEnum.ProvidingAirLaunchDevice;

  const formControls = useForm<ProvidingAirLunchDeviceFormValues>({
    mode: 'onSubmit',
  ...
  return (
    <>
      ...
      >
          ...
          <View style={{ padding: 20 }}>
            ...
            {errors.forcedDownTime ? (
              <InlineAlert type="danger">
                Чтобы начать Предоставление УВЗ, сначала отожмите кнопку «Вынужденный простой»
              </InlineAlert>
            ) : null}
        ...
        {currentStep === TenderStepperKeys.Result && <ProvidingAirLunchDeviceReport />}
      </ScrollViewContainer>
    </>
  );
};

export default observer(ProvidingAirLunchDeviceScreen);
