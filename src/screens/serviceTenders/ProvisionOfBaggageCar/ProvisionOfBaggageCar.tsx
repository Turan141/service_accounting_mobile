...
const ProvisionOfBaggageCarScreen: FC<TenderScreenProps> = ({ navigation, route }) => {
  const { currentTender, addPositionsToTender, editItemOfDocument, cancelService, changeItemStatus, loading } =
    useTendersStore();
  const { getDictionaryItemsByType, baggageCarOptions } = useDictionariesStore();
  ...
  const stepperSteps: TaskStepSchema[] = useMemo(
    () => [
      ...
      {
        order: 2,
        label: 'Предоставление спецтехники и оборудования для погрузочно-разгрузочных работ',
  ...
  const formControls = useForm<ProvisionOfBaggageCarFormValues>({
    mode: 'onSubmit',
    ...
    defaultValues: {
      ...
      machineryCount: tenderItem.properties?.machineryCount,
  ...
  useEffect(() => {
    getDictionaryItemsByType(TenderTypesEnum.ProvisionOfBaggageCar);
  }, []);
  ...
  const handleBaggageCarType = useCallback(
  ...
  return (
    <>
      ...
      >
          ...
          <View style={{ padding: 20 }}>
            ...
            {errors.forcedDownTime ? (
              <InlineAlert type="danger">
                Чтобы начать Обслуживание водяной системы, сначала отожмите кнопку «Вынужденный простой»
              </InlineAlert>
            ) : null}
              ...
              <Controller
                ...
                render={({ field: { onChange, value } }) => (
                  <Select
                    ...
                    label={'Вид работы'}
                    items={baggageCarOptions}
                    onSelect={(option: DocumentItemView) => handleBaggageCarType(value, option, onChange)}
                  />
                )}
              />
            </FormGroup>
        ...
        {currentStep === TenderStepperKeys.Result && <ProvisionOfBaggageCarReport />}
      </ScrollViewContainer>
    </>
  );
};

export default observer(ProvisionOfBaggageCarScreen);
