...
const DrainContainerScreen: FC<TenderScreenProps> = ({ navigation, route }) => {
  const { currentTender, addPositionsToTender, editItemOfDocument, cancelService, changeItemStatus, loading } =
    useTendersStore();
  const { getDictionaryItemsByType, drainContainerOptions } = useDictionariesStore();
  ...
  const formControls = useForm<DrainContainerFormValues>({
    mode: 'onSubmit',
    ...
    defaultValues: {
      ...
      litersCount: tenderItem.properties?.litersCount,
  ...
  useEffect(() => {
    getDictionaryItemsByType(TenderTypesEnum.DrainContainer);
  }, []);
  ...
  return (
    <>
      ...
      >
          ...
          <View style={{ padding: 20 }}>
            ...
            {errors.forcedDownTime ? (
              <InlineAlert type="danger">
                Чтобы начать Емкость для слива ГСМ, сначала отожмите кнопку «Вынужденный простой»
              </InlineAlert>
            ) : null}
              ...
              <Controller
                ...
                render={({ field: { onChange, value } }) => (
                  <Select
                    ...
                    label={'Тип топлива'}
                    items={drainContainerOptions}
                    onSelect={(option: DocumentItemView) => handleFuelType(value, option, onChange)}
                  />
                )}
              />
            </FormGroup>
        ...
        {currentStep === TenderStepperKeys.Result && <DrainContainerReport />}
      </ScrollViewContainer>
    </>
  );
};

export default observer(DrainContainerScreen);
