...
import OutstaffingReport from './OutstaffingReport';
...
const OutstaffingScreen: FC<TenderScreenProps> = ({ navigation, route }) => {
  const { currentTender, addPositionsToTender, editItemOfDocument, cancelService, changeItemStatus, loading } =
    useTendersStore();
  const { getDictionaryItemsByType, outstaffingOptions } = useDictionariesStore();
  ...
  const stepperSteps: TaskStepSchema[] = useMemo(
    () => [
      ...
      {
        order: 2,
        label: 'Персонал для дополнительных работ',
  ...
  const formControls = useForm<OutstaffingFormValues>({
    mode: 'onSubmit',
    ...
    defaultValues: {
      ...
      personnelCount: tenderItem?.properties?.personnelCount,
  ...
  useEffect(() => {
    getDictionaryItemsByType(TenderTypesEnum.Outstaffing);
  }, []);
  ...
  const handleOutstaffingType = useCallback(
  ...
  return (
    <>
      ...
      >
              ...
              <Controller
                ...
                render={({ field: { onChange, value } }) => (
                  <Select
                    ...
                    label={'Тип'}
                    items={outstaffingOptions}
                    onSelect={(option: DocumentItemView) => handleOutstaffingType(value, option, onChange)}
                  />
                )}
              />
            </FormGroup>
        ...
        {currentStep === TenderStepperKeys.Result && <OutstaffingReport />}
      </ScrollViewContainer>
    </>
  );
};

export default observer(OutstaffingScreen);
