...
import LaddersProvisionReport from './LaddersProvisionReport';
...
const LaddersProvisionScreen: FC<TenderScreenProps> = ({ navigation, route }) => {
  const { currentTender, addPositionsToTender, editItemOfDocument, cancelService, changeItemStatus, loading } =
    useTendersStore();
  const { getDictionaryItemsByType } = useDictionariesStore();
  ...
  const stepperSteps: TaskStepSchema[] = useMemo(
    () => [
      ...
      {
        order: 2,
        label: 'Предоставление стремянок/трапов',
  ...
  const formControls = useForm<LaddersProvisionFormValues>({
    mode: 'onSubmit',
  ...
  const handleLaddersType = useCallback(
  ...
  return (
    <>
      ...
      >
        ...
        {currentStep === TenderStepperKeys.Result && <LaddersProvisionReport />}
      </ScrollViewContainer>
    </>
  );
};

export default observer(LaddersProvisionScreen);
