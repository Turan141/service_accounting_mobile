...
import GroundPowerUnitReport from './GroundPowerUnitReport';
...
const GroundPowerUnitScreen: FC<TenderScreenProps> = ({ navigation }) => {
  const { currentTender, changeItemStatus, cancelService, editItemOfDocument, addPositionsToTender, loading } =
    useTendersStore();
  ...
  const stepperSteps: TaskStepSchema[] = useMemo(
    () => [
      ...
      {
        order: 2,
        label: 'Предоставление передвижного источника электропитания',
  ...
  const formControls = useForm<GroundPowerUnitFormValues>({
    mode: 'onSubmit',
  ...
  return (
    <>
      ...
      >
        ...
        {currentStep === TenderStepperKeys.Result && <GroundPowerUnitReport />}
      </ScrollViewContainer>
    </>
  );
};

export default observer(GroundPowerUnitScreen);
