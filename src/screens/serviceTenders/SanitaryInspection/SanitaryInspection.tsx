...
import SanitaryInspectionReport from './SanitaryInspectionReport';
import TenderCompletionPartForm from '../TenderCompletionPartForm';
import TenderInfo from '../TenderInfo';

interface SanitaryInspectionFormValues {
  [DocumentItemNamesEnum.SanitaryInspection]: DocumentItemView;
...
const SanitaryInspectionScreen: FC<TenderScreenProps> = ({ navigation }) => {
  ...
  const tenderItemName = DocumentItemNamesEnum.SanitaryInspection;

  const formControls = useForm<SanitaryInspectionFormValues>({
    mode: 'onSubmit',
  ...
  return (
    <>
      ...
      >
        ...
        {currentStep === TenderStepperKeys.Result && <SanitaryInspectionReport />}
      </ScrollViewContainer>
    </>
  );
};

export default observer(SanitaryInspectionScreen);
