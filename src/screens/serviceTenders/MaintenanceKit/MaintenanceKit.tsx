...
import MaintenanceKitReport from './MaintenanceKitReport';
...
const MaintenanceKitScreen: FC<TenderScreenProps> = ({ navigation, route }) => {
  const { currentTender, addPositionsToTender, editItemOfDocument, cancelService, changeItemStatus, loading } =
    useTendersStore();
  const { getDictionaryItemsByType, maintenanceKitOptions } = useDictionariesStore();
  ...
  const formControls = useForm<MaintenanceKitFormValues>({
    mode: 'onSubmit',
    ...
    defaultValues: {
      ...
      numberseats: tenderItem.properties?.numberseats,
      weight: tenderItem.properties?.weight,
  ...
  useEffect(() => {
    getDictionaryItemsByType(TenderTypesEnum.MaintenanceKit);
  }, []);
  ...
  return (
    <>
      ...
      >
          ...
          <View style={{ padding: 20 }}>
            ...
            {errors.forcedDownTime ? (
              <InlineAlert type="danger">
                Чтобы начать Техническая аптечка ВС, сначала отожмите кнопку «Вынужденный простой»
              </InlineAlert>
            ) : null}
              ...
              <Controller
                ...
                render={({ field: { onChange, value } }) => (
                  <Select
                    ...
                    label={'Вид работы'}
                    items={maintenanceKitOptions}
                    onSelect={(option: DocumentItemView) => handleWorkType(value, option, onChange)}
                  />
                )}
              />
            </FormGroup>
        ...
        {currentStep === TenderStepperKeys.Result && <MaintenanceKitReport />}
      </ScrollViewContainer>
    </>
  );
};

export default observer(MaintenanceKitScreen);
